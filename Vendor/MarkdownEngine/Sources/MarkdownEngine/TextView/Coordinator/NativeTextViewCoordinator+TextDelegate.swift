...
extension NativeTextViewCoordinator {
    ...
    public func textDidChange(_ notification: Notification) {
        ...
        // Keep undo entirely on AppKit's native NSTextView/UndoManager path, but
        // close the coalescing run after each committed edit. A marked-text IME
        // composition remains one transaction until the candidate is committed.
        defer { finishNativeUndoStep(after: tv) }
    ...
    private func finishNativeUndoStep(after textView: NSTextView) {
    ...
    public func textView(_ textView: NSTextView, clickedOnLink link: Any, at charIndex: Int) -> Bool {
        ...
        if let target = link as? String, target == "repotra-toc" {
            showRepotraTOC(for: textView)
            return true
        }
        if let target = link as? String, target.hasPrefix("repotra-footnote:") {
            let identifier = String(target.dropFirst("repotra-footnote:".count))
            jumpToRepotraFootnote(identifier, from: charIndex, in: textView)
            return true
        }
    ...
    private func jumpToRepotraFootnote(_ identifier: String, from location: Int, in textView: NSTextView) {
        ...
        let matches = regex.matches(in: textView.string, range: NSRange(location: 0, length: ns.length))
        let current = matches.first { NSLocationInRange(location, $0.range) }
        let currentIsDefinition = current.map { ns.substring(with: $0.range).hasSuffix(":") } ?? false
        guard let destination = matches.first(where: {
            !NSEqualRanges($0.range, current?.range ?? NSRange(location: NSNotFound, length: 0)) &&
            ns.substring(with: $0.range).hasSuffix(":") != currentIsDefinition
        }) ?? matches.first(where: { !NSEqualRanges($0.range, current?.range ?? NSRange(location: NSNotFound, length: 0)) }) else {
            return
        }
    ...
    private func showRepotraTOC(for textView: NSTextView) {
        ...
        let actionTarget = RepotraTOCMenuActionTarget(textView: textView)
        tocMenuActionTarget = actionTarget
        if matches.isEmpty {
            let item = NSMenuItem(title: "暂无标题", action: nil, keyEquivalent: "")
            ...
            for match in matches where match.numberOfRanges >= 3 {
                ...
                let item = NSMenuItem(
                    title: prefix + title,
                    action: #selector(RepotraTOCMenuActionTarget.jump(_:)),
                    keyEquivalent: ""
                )
                item.target = actionTarget
                item.representedObject = NSValue(range: match.range)
                menu.addItem(item)
            }
        }
        let location = NSApp.currentEvent.map { textView.convert($0.locationInWindow, from: nil) }
            ?? NSPoint(x: textView.visibleRect.midX, y: textView.visibleRect.midY)
...
final class RepotraTOCMenuActionTarget: NSObject {
    ...
    @objc func jump(_ sender: NSMenuItem) {
        guard let range = (sender.representedObject as? NSValue)?.rangeValue, let textView else { return }
