...
enum BlockKind: Equatable {
    ...
    case frontMatter     // leading --- ... --- YAML metadata
...
enum BlockParser {
    ...
    private static func computeBlocks(_ text: String) -> [Block] {
        ...
        while i < lines.count {
            ...
            if i == 0, line.trimmingCharacters(in: .whitespacesAndNewlines) == "---" {
                var end = 1
                while end < lines.count,
                      lineText(end).trimmingCharacters(in: .whitespacesAndNewlines) != "---" {
                    end += 1
                }
                if end < lines.count {
