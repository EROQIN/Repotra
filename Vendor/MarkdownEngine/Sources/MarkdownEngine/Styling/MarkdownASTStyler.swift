...
enum MarkdownASTStyler {
    ...
    static func styleAttributes(
        ...
        styleTyporaExtensions(ctx: ctx, into: &attrs)
        return attrs
    }
    ...
    /// Repotra's small Typora-compatible extension layer. These constructs stay
    /// in the source string; styling is caret-aware and therefore never changes
    /// copy, search, save, selection, or undo coordinates.
    private static func styleTyporaExtensions(ctx: Ctx, into attrs: inout [StyledRange]) {
        let patterns: [(String, Bool)] = [
            (#"(?m)^\[\^[^\]\r\n]+\]:"#, true),
            (#"\[\^[^\]\r\n]+\]"#, false),
            (#"(?im)^\[toc\][ \t]*$"#, false),
        ]
        for (pattern, isDefinition) in patterns {
            guard let re = try? NSRegularExpression(pattern: pattern) else { continue }
            for match in re.matches(in: ctx.text, range: ctx.fullRange) {
                let range = match.range
                if isDefinition {
                    let raw = ctx.ns.substring(with: range)
                    let identifier = String(raw.dropFirst(2).prefix { $0 != "]" })
                    attrs.append((range, [
                        .font: NSFont.systemFont(ofSize: max(9, ctx.baseFont.pointSize * 0.78), weight: .medium),
                        .foregroundColor: ctx.theme.link,
                        .link: "repotra-footnote:\(identifier)",
                    ]))
                } else if ctx.ns.substring(with: range).lowercased() == "[toc]" {
                    let paragraph = NSMutableParagraphStyle()
                    paragraph.paragraphSpacingBefore = 8
                    paragraph.paragraphSpacing = 12
                    attrs.append((range, [
                        .font: NSFont.systemFont(ofSize: max(11, ctx.baseFont.pointSize * 0.82), weight: .semibold),
                        .foregroundColor: ctx.theme.link,
                        .backgroundColor: ctx.theme.link.withAlphaComponent(0.10),
                    ...
                    let identifier = String(raw.dropFirst(2).prefix { $0 != "]" })
                    attrs.append((range, [
                        .font: NSFont.systemFont(ofSize: max(9, ctx.baseFont.pointSize * 0.72), weight: .semibold),
                        .foregroundColor: ctx.theme.link,
                        .baselineOffset: ctx.baseFont.pointSize * 0.28,
                        .link: "repotra-footnote:\(identifier)",
                    ]))
                }
            }
    ...
    private static func styleBlock(_ block: BlockNode, font: NSFont, ctx: Ctx, into attrs: inout [StyledRange]) {
        ...
        case .frontMatter(let range):
            styleFrontMatter(range: range, ctx: ctx, into: &attrs)
        case .blockLatex, .table, .blank:
            break   // NSImage rendering ported next
        }
    }

    private static func styleFrontMatter(range: NSRange, ctx: Ctx, into attrs: inout [StyledRange]) {
        let paragraph = NSMutableParagraphStyle()
        ...
        attrs.append((range, [
            .font: NSFont.monospacedSystemFont(ofSize: max(11, ctx.baseFont.pointSize * 0.78), weight: .regular),
            .foregroundColor: ctx.theme.mutedText,
            .backgroundColor: ctx.theme.mutedText.withAlphaComponent(0.065),
            .paragraphStyle: paragraph,
            .spellingState: 0,
        ]))
        guard !ctx.isActive(range) else { return }
        let first = ctx.ns.lineRange(for: NSRange(location: range.location, length: 0))
        let last = ctx.ns.lineRange(for: NSRange(location: max(range.location, NSMaxRange(range) - 1), length: 0))
        attrs.append((first, [.foregroundColor: NSColor.clear, .font: ctx.inlineMarkerFont]))
        if last.location != first.location {
            attrs.append((last, [.foregroundColor: NSColor.clear, .font: ctx.inlineMarkerFont]))
        }
    }
    ...
    private static func shrinkInactiveMarkers(in blocks: [BlockNode], ctx: Ctx, into attrs: inout [StyledRange]) {
        for block in blocks where ctx.inScope(block.range) {
            ...
            case .codeBlock, .blockLatex, .table, .frontMatter, .thematicBreak, .blank:
                break
            }
        }
    }
